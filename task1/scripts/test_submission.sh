...
echo "Entering $TECHARENA24_TASK1_DIR/task1 directory..."
cd $TECHARENA24_TASK1_DIR/task1

INPUT_TAR="$1"
echo "Testing a sample submission $INPUT_TAR"

echo "Trying to unpack the submission to $TECHARENA24_TASK1_DIR/task1/PredictionAlgorithm directory"
if [ -d "$TECHARENA24_TASK1_DIR/task1/PredictionAlgorithm" ]; then 
    echo "$TECHARENA24_TASK1_DIR/task1/PredictionAlgorithm directory already exists."
    echo "Please remove remove it and rerun this script" 
    exit 1
#    mv $TECHARENA24_TASK1_DIR/task1/PredictionAlgorithm $TECHARENA24_TASK1_DIR/task1/PredictionAlgorithm.old
fi

mkdir -p $TECHARENA24_TASK1_DIR/task1/PredictionAlgorithm && tar --strip-components 1 -xvf $INPUT_TAR -C $TECHARENA24_TASK1_DIR/task1/PredictionAlgorithm

echo "Building the submission..."
$TECHARENA24_TASK1_DIR/task1/scripts/build.sh > build.log 2>&1
echo "Building has finished. Log is available in $(pwd)/build.log"

echo "Evaluating the submission..."
$TECHARENA24_TASK1_DIR/task1/scripts/evaluate.sh -r routes/route.txt > result.log 
